...
public class Environment : MonoBehaviour
{
	...
	// some of these numbers could be made public to access them in the Unity Editor
	...
	float[] GenerateAllPoints(float[] pts)	{
		for(int i=0; i<= numTotalPoints; i++) {
			...
			if(i%basePointDiff !=0)	{
				// optimization --> if 0 or max, we're on edges, 
				// so we need "dummies" to do cubic interpolation
				int increment = i/basePointDiff;

				float x  = (float)(i%basePointDiff)/basePointDiff;
				
				//no left value -> make dummy left point
				if(increment==0)  {
					float dummy = (float)random.Next(-maxWindStrength, maxWindStrength);
					pts[i] = CubicInterpolate(dummy, pts[0], pts[basePointDiff], 
							pts[2*basePointDiff], x);
				}
				else {
					// no right value -> make dummy right point
					if(increment == numTotalPoints/basePointDiff - 1)  {
						float dummy = (float)random.Next(-maxWindStrength, maxWindStrength);
						pts[i] = CubicInterpolate(pts[numTotalPoints-2*basePointDiff], 
								pts[numTotalPoints-basePointDiff], 
								pts[numTotalPoints], dummy, x);
					}
					// all other values
					else {

						pts[i] = CubicInterpolate(pts[(increment-1) * basePointDiff], 
							pts[increment * basePointDiff], pts[(increment+1)*basePointDiff],  
							pts[(increment+2)*basePointDiff], x);
					}
				}
			}
		}
	...
	float[] Smooth(float[] vals) {
		for(int i=0; i<=numTotalPoints; i++) 	{
			...
			else if(i==numTotalPoints) // no i+1
				vals[i] = vals[i]/2  +  vals[i-1]/2;
			else // normal case
